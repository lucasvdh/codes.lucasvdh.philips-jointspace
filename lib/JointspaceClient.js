...
class JointspaceClient {
  ...
  setConfig (ipAddress, apiVersion = null, user = null, pass = null, secure = null, port = null) {
  ...
  static createSignature (base64Key, secret, algorithm) {
    return crypto.createHmac(algorithm, Buffer.from(base64Key, 'base64'))
      .update(secret)
      .digest('base64');
  }

  async confirmPair (pin, signatureAlgorithms = ['sha1', 'sha256', 'sha512']) {
    let authTimestamp = this.timestamp + pin,
      user = this.deviceId,
      pass = this.authKey
    ...
    return new Promise((resolve, reject) => {
      ...
      for (let signatureAlgorithm in signatureAlgorithms) {
        let signature = JointspaceClient.createSignature(secretKey, authTimestamp),
          authData = {
            'device': this.device,
            'auth': {
              'auth_AppId': '1',
  ...
  getSystem () {
    ...
    return this.request('system', 'GET', {}, {}, 1925, false, false)
  }
