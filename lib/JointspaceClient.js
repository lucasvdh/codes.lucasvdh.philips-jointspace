...
class JointspaceClient {
    ...
    setConfig(ipAddress, apiVersion, user = null, pass = null) {
    ...
    request(route, method, data = {}, headers = {}) {
        let protocol = JointspaceClient.getProtocolByApiVersion(this.apiVersion),
            host = this.ipAddress,
        ...
        return new Promise((resolve, reject) => {
            let callback = (error, response, data) => {
                if (!error && response.statusCode == 200) {
                    ...
                    console.error("Command to Philips TV, gives an error in response", error)
                    reject(error);
                }
            };

            if (this.user !== null && this.apiVersion >= 6) {
