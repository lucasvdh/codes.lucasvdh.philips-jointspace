...
class JointspaceClient {
    ...
    getInfo() {
		if(!this.notifyChangeObject) {
			this.notifyChangeObject = require('/assets/json/notifyChange.json');
			this.notifyChangeObject.notification.companionlauncher.device_id = this.deviceId;
		}
        return this.request('notifychange', 'POST', this.notifyChangeObject);
    }
}
