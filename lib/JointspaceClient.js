...
class JointspaceClient {
    ...
    */
    mute() {
        return new Promise((resolve, reject) => {
            this.getAudioData().then((response) => {
                this.setVolume(response.current, true).then(resolve).catch(reject);
            }).catch(reject);
        })
    }
    ...
    unMute() {
        return new Promise((resolve, reject) => {
            this.getAudioData().then((response) => {
                this.setVolume(response.current, false).then(resolve).catch(reject);
            }).catch(reject);
        })
    }
