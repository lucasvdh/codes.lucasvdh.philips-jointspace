...
class JointspaceClient {
    ...
    setConfig(ipAddress, apiVersion, user = null, pass = null, secure = null) {
    ...
    static getPortByApiVersion(apiVersion, secure) {
        return secure === false ? 1925 : (apiVersion >= 6 ? 1926 : 1925);
    }
    ...
    startPair() {
        ...
        return new Promise((resolve, reject) => {
            client.request('pair/request', 'post', data).then(function (json) {
                client.authKey = json.auth_key;
                client.timestamp = json.timestamp;
                client.timeout = json.timeout;
                resolve(json);
            }, function (error) {
                console.error('startPair() error', error);
                reject(error);
            });
        })
    }
    ...
    static createSignature(secretKey, secret) {
        let hmac = crypto.createHmac('sha1', new Buffer(secretKey, 'utf8'));
        hmac.write(secret);
        hmac.end();
     ...
     * @returns {Promise}
     */
    request(route, method, data = {}, headers = {}) {
        let protocol;
        ...
        if (this.secure === true) {
            ...
            protocol = JointspaceClient.getProtocolByApiVersion(this.apiVersion)
        }

        let host = this.ipAddress,
            path = '/' + this.apiVersion + '/' + route,
            port = JointspaceClient.getPortByApiVersion(this.apiVersion, this.secure),
        ...
        return new Promise((resolve, reject) => {
            let callback = (error, response, data) => {
                if (!error && response.statusCode === 200) {
                    ...
                    console.error("JointspaceClient response error", JSON.stringify(error))
                    reject(response);
                } else if (error !== null) {
                    console.error("JointspaceClient response error", JSON.stringify(error), data)
                    reject(error);
                } else {
                    console.error("JointspaceClient response error", JSON.stringify(response), data)
                    reject(response);
                }
            };
    ...
    getSystem() {
        return this.request('system', 'GET');
    }
